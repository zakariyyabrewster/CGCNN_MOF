...
# ─── CONFIG ────────────────────────────────────────────────────────────────────
config_path = "config_kcv_transformer.yaml"
with open(config_path, "r") as f:
    config = yaml.full_load(f)

result_path_pre = "training_results/finetuning/Transformer_CV"
...
def load_test_results():
    ...
    for prop in properties:
        ...
        for fold in range(n_folds):
            file_path = (
                f"{result_path_pre}/Trans_fold_{fold}_{prop}"
...
def create_comprehensive_plots(fold_df, data_name, out_dir="training_results/analysis"):
    ...
    # Save plot
    out_path = f"{out_dir}/{data_name}_trans_cv_comprehensive_analysis.png"
...
def create_ranking_plot(fold_df, data_name, out_dir):
    ...
    plt.tight_layout()
    out_path = f"{out_dir}/{data_name}_trans_cv_ranking.png"
    plt.savefig(out_path, dpi=300, bbox_inches='tight')
    plt.show()
